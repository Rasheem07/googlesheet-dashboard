'use client'
import { AudioWaveform, Box, ChartLine, Command, GalleryVerticalEnd, Group, Hammer, Settings } from "lucide-react";
  ...
  return (
    <Sidebar collapsible="icon" className="border-r border-r-gray-300">
      <SidebarHeader className="flex gap-x-4">
        <SidebarTrigger className="self-end bg-blue-500 hover:bg-blue-600"/>
      ...
      <SidebarContent>
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {items.map((item) => (
                <SidebarMenuItem key={item.title} onClick={() => setIsActive(item.title)}>
                  <SidebarMenuButton asChild isActive={isActive === item.title || item.title === "Dashboard"}>
                    <a href={item.url}>
                      <item.icon />
